...
class Allsvenskan extends Component {
    ...
    // Öppnar edit-modalen
    openEditModal = id => {
        const updatedPost = this.props.forum.allsvenskanPosts.find(post => id === post._id);
        this.setState({
    ...
    // Uppdaterar inlägg
    updatePost = () => {
        this.props.onUpdateForumPost({ id: this.state.updatedPost._id, text: this.state.updatedText, forumType: 'allsvenskan' });
    ...
    componentDidMount() {
        ...
        if (this.props.forum.currentForum !== 'allsvenskan') {
            this.props.onSwitchForumNav('allsvenskan');
        }

        // Laddar in senaste inläggen
        this.props.onFetchForumPosts('allsvenskan');

        // Skapar tidsintervall för uppdatering
        const intervalID = setInterval(() => { this.props.onFetchForumPosts('allsvenskan'); }, 30000);

        this.props.onSetInterval('allsvenskan', intervalID);

        // Sätter sidan till 1
        this.props.onSetCurrentForumPage(1, 'allsvenskan');
    }
        ...
        return (
            <Container style={{ marginTop: '3em', position: 'relative' }}>
                ...
                <Row>
                    <Col>
                        <Pagination>
                            {this.props.forum.allsvenskanPostPages ? this.props.forum.allsvenskanPostPages.map(postPage => <Pagination.Item key={postPage} onClick={(forumPageId, forumType) => this.props.onSetCurrentForumPage(postPage, 'allsvenskan')} active={this.props.forum.currentAllsvenskanForumPage === postPage}>{postPage}</Pagination.Item>) : null}
                        </Pagination>
                    </Col>
                </Row>
                <Row>
                    <Col>
                        {spinner}
                        <ListGroup>
                            {this.props.forum.allsvenskanPaginatedPostsToShow ? this.props.forum.allsvenskanPaginatedPostsToShow.map(ap => ap ? (
                                <ListGroup.Item key={ap._id}>
                                    <div className="d-flex w-100 justify-content-between">
                                        <h5 className="mb-1">{ap.nickname ? ap.nickname : ap.userName}
                                        {ap.favoriteclub ? <Badge style={{ marginLeft: '5px', fontSize: '12px' }} pill variant="info">
                                            {ap.favoriteclub}
                                        </Badge> : null}
                                        </h5>
                                        <small>{ap.updatedDate ? <span style={{ color: 'red' }}>(Redigerad {moment(ap.updatedDate).format('YYYY-MM-DD HH:mm')})</span> : null} {moment(ap.date).format('YYYY-MM-DD HH:mm')}</small>
                                    </div>
                                    <div className="d-flex w-100 justify-content-start">
                                        <p className="mb-1">{ap.text}</p>
                                    </div>
                                    {this.props.auth.user.id === ap.userId ? (<ButtonToolbar>
                                        <Button variant="info" size="sm" style={{ marginRight: '1em' }} onClick={id => this.openEditModal(ap._id)}>
                                            Redigera
                                    </Button>
                                        <Button variant="danger" size="sm" onClick={id => this.openDeleteModal(ap._id)}>
                                            Radera
                                    </Button>
                                    </ButtonToolbar>) : null}
                                </ListGroup.Item>
